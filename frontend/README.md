...
### 3. 진행률 표시
- 업로드 → 서버 변환 → 다운로드 → 마무리 4단계 타임라인
- 업로드·다운로드는 실제 네트워크 전송량 기반 퍼센트 반영
- 경과 시간, 단계별 상태 메시지 실시간 갱신

### 4. 결과 표시
- 결과 요약 카드: 형식, 해상도 변화, 용량 변화, 소요 시간, 압축률 등
- 대용량 파일은 즉시 미리보기를 지연시키고 경고/로드 버튼 제공
- 소형 파일은 자동 미리보기(지연 로딩), 다운로드 & 다시 변환 버튼
- 변환 메타데이터는 백엔드 응답 헤더(`X-Converted-Width`, `X-Process-Time` 등) 기반으로 표시
