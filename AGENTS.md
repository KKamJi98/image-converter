...
- 새로운 작업을 시작하기 전에 항상 `git pull`로 원격 변경 사항을 동기화한 뒤 작업을 진행한다.
- 프론트엔드 테스트 실행 시 watch 모드로 진입하지 않도록 `CI=true npm test -- <pattern>` 혹은 `npm test -- <pattern> -- --watch=false`를 사용해 CLI 대기 상태를 방지한다.
